...
def init_vlm():
    client = Anthropic(api_key='Enter your API')
    return client
...
if __name__ == "__main__":
    ...
    parser.add_argument('--directory', type=str, default='./gowitness/screenshots/',help='The directory to search in.')
    ...
    screenshots = read_screenshots(args.directory, args.extension)
    ...
    print('Extracting content from the images.')
    ocr = screenshots_data.map(lambda x: {"content": extract_text_from_image(args.directory + x['image_path'])})
    ocr.to_csv("./results/All_ocr_extracted.csv")
    ocr.to_json("./results/All_ocr_extracted.json")
    print('Content extraction from the images completed.')
    
    ## Call the interesting and don't bothering filter functions from Justin's code and filter on those flags
    print('Filtering on the interesting and don\'t bother flags.')
    interesting = scorer.load_keywords(args.interesting_file)
    dont_bother = scorer.load_keywords(args.dont_bother_file)
    ...
    model, tokenizer, nlp_pipe = init_ner_model(args.ner_model)
    ...
    # entitiy_ocr.to_json("./results/entity_ocr.json")
    # entitiy_ocr = datasets.load_dataset("json", data_files = "./results/entity_ocr.json", split = "train")
    # entitiy_ocr = entitiy_ocr.select(range(20))
    client = init_vlm()
    analysis_ocr_vlm = entitiy_ocr.map(lambda x: get_analysis_and_vlm_score(x, prompt, client, args.directory,args.vlm_model))
    analysis_ocr_vlm.to_csv("./results/analysis_ocr_vlm.csv")
    analysis_ocr_vlm.to_json("./results/analysis_ocr_vlm.json")
    ...
    average_score_ocr_vlm_cleaned.to_csv("./results/All_attack_portals.csv")
    average_score_ocr_vlm_cleaned.to_csv("./results/All_attack_portals.json")
    
    top_k_instances = average_score_ocr_vlm_cleaned.select(range(min(args.top, len(average_score_ocr_vlm_cleaned))))
    ## save and print the information to a CSV format
    printing_function(top_k_instances)
    top_k_instances.to_csv("./results/Best_possible_attack_portals.csv")
    top_k_instances.to_json("./results/Best_possible_attacks.json")
    
    # return_ordered_list_with_screenshot()
